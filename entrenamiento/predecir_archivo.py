...
"""def predecir_y_evaluar_lstm(modelo, label_encoder, datos_prueba, nombre_modelo, carpeta_salida, umbrales, nombre_base):
    ...
    try:
        ...
        #X_principal = datos_prueba[columnas_requeridas].values.astype('float32')
        # Procesar joints (34 pasos temporales, 1 característica)
        ...
        y_pred = []
        confianzas = []
        historial_confianzas = deque(maxlen=longitud_secuencia)

        # Primera pasada: generar predicciones iniciales
        for prob in probabilidades:
            historial_confianzas.append(prob)            
            # Calcular confianza ponderada temporalmente
            pesos = np.linspace(0.1, 1.0, len(historial_confianzas))
            prob_ponderada = np.average(historial_confianzas, axis=0, weights=pesos)            
            clase_seleccionada = None
            max_confianza = -1            
            # Estrategia de umbrales adaptativos
            for clase_id, (umbral_min, umbral_max) in umbrales.items():
                confianza = prob_ponderada[clase_id]                
                # Umbral dinámico basado en contexto
                umbral_ajustado_min = umbral_min * (1 - 0.1*len(historial_confianzas)/longitud_secuencia)
                umbral_ajustado_max = umbral_max * (1 + 0.1*len(historial_confianzas)/longitud_secuencia)                
                if (umbral_ajustado_min <= confianza <= umbral_ajustado_max) and (confianza > max_confianza):
                    clase_seleccionada = clase_id
                    max_confianza = confianza
            
            # Fallback con memoria temporal
            if clase_seleccionada is None:
                avg_prob = np.mean(historial_confianzas, axis=0)
                clase_seleccionada = np.argmax(avg_prob)
                max_confianza = avg_prob[clase_seleccionada]            
            y_pred.append(clase_seleccionada)
            confianzas.append(max_confianza)
                      
        # Generar etiquetas iniciales
